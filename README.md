...
dex = dexgen.Dexgen()
```
Run ```python run.py``` once to create directories<br />Put files in ```/to_add``` folder<br />Run ```python run.py``` twice<br />```/gen/index.html``` (in the ```/gen``` folder) is your website
